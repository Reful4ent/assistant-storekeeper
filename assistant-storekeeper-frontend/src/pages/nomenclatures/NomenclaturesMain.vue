<script setup>
  ...
  const getNomenclatures = async () => {
    try {
      ...
      const response = await axios.get(apiUrl + '/api/nomenclatures', { params });
      nomenclatures.value = response.data.data;
  ...
  const onEditClick = (id) => {
    router.push(`/nomenclatures/${id}/edit`);
  }

  const onAddClick = () => {
    router.push('/nomenclatures/create');
  }

  const onDeleteClick = async (id) => {
    try {
      const response = await axios.delete(apiUrl + `/api/nomenclatures/${id}`);
      if (response.status === 204) {
        message.success('Номенклатура успешно удалена');
        await getNomenclatures();
      }
    } catch (error) {
      message.error('Не удалось удалить номенклатуру');
    }
  }
